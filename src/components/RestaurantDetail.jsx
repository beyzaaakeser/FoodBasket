...
const RestaurantDetail = ({data}) => {
    ...
    return (
        <div className="flex p-5 rounded-md max-sm:flex-col gap-6 mt-8">
            <img src={data.photo} alt={data.name} className="img-fluid max-sm:w-full max-md:w-[350px] w-[400px] h-[250px] object-cover rounded-md shadow" />
            ...
            <div className='flex flex-col gap-3 justify-between'>
                ...
                <p className="flex items-center gap-6 font-semibold text-gray-600">
                    <span className="flex items-center gap-2 font-semibold text-gray-600"> <IoStar
                        className="text-amber-600 text-xl"/> {data.rating} </span>
                    <span className="flex items-center gap-2 font-semibold text-amber-500">
                       <a href="#" className="p-1 px-3 rounded hover:bg-amber-400 hover:text-gray-600 transition ">Yorumlar</a>
                   </span>
                </p>
            </div>
        </div>
