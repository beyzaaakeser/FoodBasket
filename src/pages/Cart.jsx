...
const Cart = () => {
    ...
    const totalBasketPrice = cart.data.reduce((a, b) => a + b.amount * b.price, 0).toFixed(2);
    ...
    return (
        <Container>
            <h1 className="text-2xl font-bold mt-10 my-5 text-center mx-auto">SEPETINIZ</h1>
            <div>
                {cart.isLoading ? (
                    <Loading/>
                ) : cart.error ? (
            ...
            {cart.data.length !== 0 &&
                <div
                    className=' max-w-[1280px] bg-white m-auto rounded-md border-t border-b-gray-400 p-4 flex flex-col gap-3 '>
                    ...
                    <div className='border-b border-gray-200 py-3 flex justify-between '>
                        <span className='font-bold text-gray-600 text-xl'>Total basket price</span>

                        <span className='font-bold text-xl'>{(totalBasketPrice)}
